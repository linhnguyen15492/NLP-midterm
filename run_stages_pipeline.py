...
class ExtractionStage:
    ...
    def run(self, page_num: int, output_json_path: Path) -> Dict:
        ...
        """
        print(f"[Stage 1: Extraction] Processing page {page_num} of {self.pdf_path.name}")
        ...
        extracted_lines.sort(key=lambda x: sum(c["bbox"][1] for c in x["chars"]) / len(x["chars"]))
...
class AlignmentStage:
    ...
    def simulate_ocr_errors(self, chars: List[Dict], error_rate: float = 0.1) -> List[Dict]:
    ...
    def run(self, input_json_path: Path, output_json_path: Path) -> List[Dict]:
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            correct_count = sum(1 for c in aligned_chars if c["status"] in ("black", "green"))
...
class ExportStage:
    ...
    def run(self, input_json_path: Path, output_xml_path: Path, output_xlsx_path: Path):
        ...
        ws.append(["STC_ID", "Page", "OCR Char", "Corrected Char", "Status", "xmin", "ymin", "xmax", "ymax", "Confidence"])
        ...
        for record in aligned_records:
            ...
            for char_info in chars:
                ...
                # XML node
                ET.SubElement(bboxes_node, "BBOX", {
                ...
                # Excel row
                ws.append([
                    stc_id, page_num, orig_c, corr_c, status,
...
def main():
    ...
    # Builders
    id_builder = STCIDBuilder(domain="H", sub_domain="V", genre="B", file_num=3, chapter=1, page=page_num)

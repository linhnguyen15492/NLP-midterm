...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Staged extractor for text-layer PDFs (Vietnamese side)")
    p.add_argument("--pdf", type=Path, default=Path(r"data/vie/An_Nam_Chi_Luoc.pdf"))
    p.add_argument("--out-dir", type=Path, default=Path(r"output/vie/an_nam_chi_luoc_stages"))
    p.add_argument("--file-id", type=str, default="HVB_001")
    p.add_argument("--lang", type=str, default="V", help="Language code for alignment_input.json (V or C)")
...
def main() -> None:
    ...
    pages = extract_text_blocks_by_page(args.pdf, page_start=args.page_start, page_end=args.page_end)
    write_json({"pdf": str(args.pdf), "pages": pages}, interim / "stage_01_extract.json")

    # Stage 02: clean
    pages_clean = clean_page_text(pages, cleaner=cleaner)
    write_json({"pdf": str(args.pdf), "pages": pages_clean}, interim / "stage_02_clean.json")

    # Stage 03: sentences
    sentences = split_pages_to_sentences(pages_clean, splitter=splitter, id_builder=id_builder)
    write_json({"pdf": str(args.pdf), "sentences": sentences}, interim / "stage_03_sentences.json")

    # Stage 04: alignment input (pair this with Han-side JSON of same schema)
    alignment_input = export_alignment_input(sentences, file_id=args.file_id, lang=args.lang)
    ...
    export_viet_xml(out_xml, file_id=args.file_id, meta=meta, id_builder=id_builder, sentences=sentences)

...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Extract intermediate artifacts from scan-based Vietnamese PDFs"
    )
    p.add_argument(
        "--pdf",
        type=Path,
        default=Path(r"data/sino/An_Nam_Chi_Nguyen_sino.pdf"),
        help="Input PDF path",
    )
    p.add_argument(
        "--out-dir",
        type=Path,
        default=Path(r"data/interim/sino/An_Nam_Chi_Nguyen_sino"),
        help="Output directory for intermediate artifacts",
    )
    p.add_argument("--dpi", type=int, default=400, help="Render DPI for page PNGs")
    p.add_argument("--page-start", type=int, default=1, help="First page (1-based)")
    p.add_argument(
        "--page-end",
        type=int,
        default=5,
        help="Last page (1-based). Use a large number for full run.",
    )
